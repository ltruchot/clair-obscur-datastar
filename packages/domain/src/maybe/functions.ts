import type { Catamorphism } from './Catamorphism.ts';
 ...
 */
export function isNothing<T>(maybe: Maybe<T>): maybe is Maybe<T> & { state: null } {
 ...
 */
export function traverse<T, U>(fn: (value: T) => Maybe<U>): (values: T[]) => Maybe<U[]>;
export function traverse<T, U>(fn: (value: T) => Maybe<U>, values: T[]): Maybe<U[]>;
...
export function filter<T>(fn: (value: T) => boolean, maybe?: Maybe<T>) {
  const doit = (maybe: Maybe<T>) => maybe.andThen((x) => (fn(x) ? just(x) : nothing()));
  return typeof maybe === 'undefined' ? doit : doit(maybe);
}
 ...
 */
export function ap<T, U>(maybeFn: Maybe<(x: T) => U>, maybe: Maybe<T>): Maybe<U>;
export function ap<T, U>(maybeFn: Maybe<(x: T) => U>): (maybe: Maybe<T>) => Maybe<U>;
